...
int find_max_num_index(struct compx *data, int count)
{
    ...
    // 调试: 打印初始最大值和初始索引值
    //rt_kprintf("Initial max value: %d at index %d\n", (int)(1000*temp), i);
    ...
    for (i = START_INDEX; i < count; i++)
    {
        if (temp < data[i].real)
        {
            ...
            max_num_index = i;
            // 调试: 新找到的最大值和索引
            //rt_kprintf("New max value: %d at index %d\n", (int)(1000*temp), i);
        }
    }

    // 返回最大值所在索引
    //rt_kprintf("Returned max value: %d at index %d\n", (int)(1000*temp), max_num_index);
...
/**
 * @brief 实现Butterworth带通滤波器
 * 
 * @param new_data 新的输入数据
 * @param prev_output1 上一次的输出数据1
 * @param prev_output2 上一次的输出数据2
 * @param prev_input1 上一次的输入数据1
 * @param prev_input2 上一次的输入数据2
 * @param center_freq 滤波器的中心频率
 * @param bandwidth 滤波器的带宽
...
void ButterworthBandPassFilter(float new_data, float *prev_output1, float *prev_output2, float *prev_input1, float *prev_input2, float center_freq, float bandwidth, float sample_rate, float *b, float *a) {
    float omega = 2.0f * XPI * center_freq / sample_rate;
    float delta = 2.0f * XPI * bandwidth / sample_rate;
    float alpha = sin(omega) * sinh(0.5f * delta);
    ...
    float output = (b[0] * new_data + b[1] * (*prev_input1) + b[2] * (*prev_input2)) / a[0] - a[1] * (*prev_output1) / a[0] - a[2] * (*prev_output2) / a[0];

    *prev_input2 = *prev_input1;
    *prev_input1 = new_data;
