...
#include <rtthread.h>
#include <rtdevice.h>
#include <./drivers/include/drv_algorithm.h>
#include <math.h>
...
Physiological_signal blood_result;
...
static rt_uint32_t g_heartrate = 66;
...
{
    /* 如果未分配内存，则在此分配 */
    if (my_data_p == RT_NULL)
    {
        my_data_p = rt_malloc(sizeof(max30102_data));
        if (my_data_p == RT_NULL) {
            rt_kprintf("Failed to allocate memory for sensor data.\n");
            return -RT_ERROR;
        }
    }
...
max30102_data_ptr my_data_p = RT_NULL;
int16_t last_data_red,last_data_ir;
static void max30102_thread_entry(void *args)
{
    rt_sensor_t sensor = args;
    uint32_t pun_red_led, pun_ir_led;
    uint32_t last_red, last_ir;
    my_data_p->red = 0;
    my_data_p->ir = 0;
    my_data_p->get_finish_flag = 0;
    static float spO2_get,SPO2,spO2_get_count,spo2_sum;
    ...
    while (1)
    {
        ...
        if (tick - sample_tick >= 1000 / FS)      //FS Hz sample rate
        {
            if (rt_pin_read(sensor->config.irq_pin.pin) == 0)
            {
                ...
                my_data_p->red = pun_red_led - last_red;
                my_data_p->ir = pun_ir_led - last_ir;
                my_data_p->get_finish_flag = 1;

                //限幅
                if(my_data_p->ir > 600 || my_data_p->ir < -1000)
                    my_data_p->ir = last_data_ir;

                // Update threshold based on IR data
                if (my_data_p->ir < threshold)
                {
                    min = MIN(min, my_data_p->ir);
                }
                else if (min < 0)
                {
                ...
                rt_kprintf("<max30102>: %d, %d, %d, %d, %d\n", my_data_p->red, my_data_p->ir, threshold, g_heartrate, (int16_t)SPO2);
                ...
                if (init_flag)
                {
                    ...
                    if (0 == interval_average_tick)
                        ...
                        if(g_heartrate > 140)
                            g_heartrate = 140;
                        else if(g_heartrate < 40)
                            g_heartrate = 40;

                        blood_result.HR = g_heartrate;
                        }
                ...
                // 计算血氧饱和度（SpO2）
                float dc_red = my_data_p->red;
                float dc_ir = my_data_p->ir;
                float ac_red = fabs(pun_red_led - dc_red);
                float ac_ir = fabs(pun_ir_led - dc_ir);

                float R = (ac_red * dc_ir) / (ac_ir * dc_red);
                spO2_get = 135.0f - 25.0f * R; // 25

                // 对SpO2值进行范围限制
                if (spO2_get < 93.0f) spO2_get = 93.0f;
                if (spO2_get > 99.0f) spO2_get = 99.0f;
                spo2_sum += spO2_get;
                spO2_get_count++;
                if(spO2_get_count >= 100)
                {
                    spO2_get_count = 0;
                    SPO2 = spo2_sum / 100;
                    spo2_sum = 0;
        ...
        last_data_red = my_data_p->red;
        last_data_ir = my_data_p->ir;
